ARG RUST_TAG=1.92-slim
FROM debian:bookworm-slim as fetcher
RUN apt-get update && \
    apt-get install -y wget

RUN wget https://github.com/rustsec/rustsec/releases/download/cargo-audit%2Fv0.22.0/cargo-audit-x86_64-unknown-linux-musl-v0.22.0.tgz && \
    tar -xvf cargo-audit-x86_64-unknown-linux-musl-v0.22.0.tgz && \
    mv cargo-audit-x86_64-unknown-linux-musl-v0.22.0/cargo-audit /usr/local/bin/

RUN wget https://github.com/casey/just/releases/download/1.15.0/just-1.15.0-x86_64-unknown-linux-musl.tar.gz && \
    tar -xvf just-1.15.0-x86_64-unknown-linux-musl.tar.gz && \
    chmod +x just && \
    mv just /usr/local/bin/

FROM rust:$RUST_TAG
ARG RUST_TAG=1.92-slim

RUN apt-get update && \
    apt-get install -y wget make perl musl-tools && \
    rustup component add rustfmt && \
    rustup component add clippy && \
    rustup target add x86_64-unknown-linux-musl

COPY --from=fetcher /usr/local/bin/cargo-audit /usr/local/bin/cargo-audit
COPY --from=fetcher /usr/local/bin/just /usr/local/bin/just
